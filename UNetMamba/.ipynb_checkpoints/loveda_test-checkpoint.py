...
def main():
    ...
    # model = Supervision_Train.load_from_checkpoint(os.path.join("UNetMamba",config.weights_path, config.test_weights_name+'.ckpt'), config=config)
    # model = Supervision_Train.load_from_checkpoint("UNetMamba/model.pth", config=config)
    model=torch.load("UNetMamba/final.pth")
    print(model)
    model.cuda()
    model.eval()
    if args.tta == "lr":
