...
class CoordinateAttention(nn.Module):
    def __init__(self, in_channels, reduction=1):
        ...
        # 横向和纵向的坐标注意力分支
        self.horizontal = nn.Conv2d(in_channels, in_channels // reduction, kernel_size=1)
        self.vertical = nn.Conv2d(in_channels, in_channels // reduction, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 横向注意力分支
        x_h = x.mean(dim=2, keepdim=True)  # 按照H方向平均
        x_h = self.horizontal(x_h)  # 横向卷积操作
        x_h = x_h.view(B, C // self.reduction, 1, W)  # Shape: [B, C//reduction, 1, W]
        x_h = self.sigmoid(x_h)  # Sigmoid激活
        
        # 纵向注意力分支
        x_v = x.mean(dim=3, keepdim=True)  # 按照W方向平均
        x_v = self.vertical(x_v)  # 纵向卷积操作
        x_v = x_v.view(B, C // self.reduction, H, 1)  # Shape: [B, C//reduction, H, 1]
        x_v = self.sigmoid(x_v)  # Sigmoid激活
        
        # 将两个方向的注意力图相乘
        out = x * x_h * x_v  # 融合坐标信息
...
#     # 假设输入图像大小为 (B, C, H, W)
#     # x = torch.randn(4, 64, 32, 32)  # B=4, C=64, H=32, W=32
#     # coord_attention = CoordinateAttention(in_channels=64)
#     # out = coord_attention(x)
#     # print(out
#     model=UNetMamba()
#     print(model)
